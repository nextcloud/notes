OC.L10N.register(
    "notes",
    {
    "Notes" : "Заметки",
    "New note" : "Новая заметка",
    "Updating the note's category has failed. Is the target directory writable?" : "Ошибка обновления категории заметки. Разрешена ли запись в целевую директорию?",
    "Updating the note's category has failed." : "Ошибка обновления категории заметки.",
    "There are unsaved notes. Leaving the page will discard all changes!" : "Заметка не сохранены. Если покинуть страницу, все изменения будут удалены!",
    "Please wait while new settings are applied…" : "Подождите, пока будут применены новые настройки ...",
    "_%n word_::_%n words_" : ["%n слово","%n слова","%n слов","%n слов"],
    "The last viewed note cannot be accessed. " : "Не удалось перейти к последней просмотренной заметке",
    "The notes folder is not accessible: %s" : "Папка заметок недоступна: %s",
    "File error" : "Ошибка файла",
    "Encryption Error" : "Ошибка шифрования",
    "Error" : "Ошибка",
    "Distraction-free notes and writing" : "Простые заметки и записки",
    "Uncategorized" : "Без категории",
    "Categories" : "Категории",
    "All notes" : "Все заметки",
    "Search result for \"{{search}}\" in {{SelectedCategory}}" : "Результаты поиска по запросу \"{{search}}\" в категории {{SelectedCategory}}",
    "Search result for \"{{search}}\"" : "Результаты поиска по запросу \"{{search}}\"",
    "No search result for {{search}} in {{SelectedCategory}}" : "Нет результатов поиска для {{search}} в категории {{SelectedCategory}}",
    "No search result for {{search}}" : "Нет результатов поиска для {{search}}",
    "No notes found" : "Записей не найдено",
    "Delete note" : "Удалить заметку",
    "Favorite" : "Избранное",
    "Settings" : "Параметры",
    "Folder to store your notes" : "Папка для хранения заметок",
    "path to notes" : "путь к заметкам",
    "File extension for new notes" : "Расширения файла для новых заметок",
    "Category" : "Категория",
    "Edit category" : "Редактировать категорию",
    "The note has unsaved changes." : "Изменения в заметке еще не сохранены.",
    "Click here to try again" : "Нажмите здесь для повтора.",
    "Saving failed!" : "Ошибка при сохранении!",
    "Note saved" : "Заметка сохранена"
},
"nplurals=4; plural=(n%10==1 && n%100!=11 ? 0 : n%10>=2 && n%10<=4 && (n%100<12 || n%100>14) ? 1 : n%10==0 || (n%10>=5 && n%10<=9) || (n%100>=11 && n%100<=14)? 2 : 3);");
