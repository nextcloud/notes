...
Since Notes 6.1, attachments are stored in a note-specific folder and can be deleted via the API. This did not introduce a new API version; check the app `version` capability (see [README](README.md#capabilites)) if you depend on it. Attachment endpoints can be reached with both base paths `/api/v1/` and `/api/v1.4/`.
